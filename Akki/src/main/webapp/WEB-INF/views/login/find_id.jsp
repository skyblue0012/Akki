...
<html>
	...
	<title>삼천리악기</title>
		...
		<link href="${pageContext.request.contextPath}/resources/css/login/find_id.css" rel="stylesheet">
		<script type="text/javascript">
			...
			function checking_info(f) {
				...
				if(document.getElementById("now").value == "1"){
					...
					var url = "checking_id.do";
					var param = "name=" + f.name.value + "&email=" + f.email.value + "&tel=" + "no";
					sendRequest(url, param, check_id_m, "POST");
					...
					function check_id_m() {
						if (xhr.readyState == 4 && xhr.status == 200) {
							...
							if (data == "no") {
								...
								document.getElementById('res_id').value = data;
							}
						}
					}
				}else{
					if(document.getElementById("phone1").value==""||document.getElementById("phone2").value==""||document.getElementById("phone3").value==""){
						...
						var url = "checking_id.do";
						var param = "name=" + f.name.value + "&email=" + "no" + "&tel=" + tel;
						
						sendRequest(url, param, check_id_t, "POST");
			...
			function login_page() {
				location.href = "login_page.do?id=" + document.getElementById("res_id").value;
			}
			
			function find_pwd() {
				location.href = "find_pwd.do?id=" + document.getElementById("res_id").value;
			}
	...
	<body>
		...
		<div id="result_box">
			<p id="find_id">아이디 찾기</p>
			<p id="coment">아이디 찾기를 완료하였습니다</p>
			<p id="coment_id">아이디</p>
			<input type="text" id="res_id" readonly>
			<input type="button" id="log_in" value="로그인" onclick="login_page()">
			<input type="button" id="log_in" value="비밀번호찾기" onclick="find_pwd()">
		</div>
